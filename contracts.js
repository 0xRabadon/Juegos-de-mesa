...
export const JuegoCreateInput = z.object({
  // Agregamos 'id' e 'imagen' como opcionales para que pasen la validación
...
}).passthrough(); // passthrough permite campos extra, pero es mejor definirlos arriba

// 2. DEFINICIÓN DE SALIDA (LO QUE VE EL FRONTEND)
export const JuegoDTO = z.object({
  id: z.string().nullable().optional(), // Útil tener el ID
  imagen: z.string().nullable(),        // <--- ¡ESTO ES LO QUE FALTABA!
...
export function toJuegoDTO(input) {
  ...
  // Construimos el objeto intermedio
  const dto = {
    id: data.id ?? null,
    imagen: data.imagen ?? null, // <--- Pasamos la imagen aquí
  ...
  // Validamos la salida final con el filtro DTO
